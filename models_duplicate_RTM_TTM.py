...
class LightweightSTM(nn.Module):
    def __init__(self):
        super(LightweightSTM, self).__init__()

    def forward(self, x):
        """
        x shape: [batch_size, timesteps, channels, embedding_dim]
        STM expects: [batch_size, channels, timesteps, embedding_dim]
        """
        # 🔄 **Rearrange the tensor to match the original STM format**
        x = x.permute(0, 2, 1, 3)  # Swap timesteps and channels
        return x
...
class TTM(nn.Module):  # Temporal Transformer Module
    def __init__(self, input, num_submatrices, num_blocks, num_heads, dtype):
        ...
        self.dtype = dtype
        self.avgf = num_submatrices  # Number of submatrices (M)
        self.input = input.transpose(1, 3)  # Transform shape to [D, S, C]
        self.seg = self.input.shape[1] / self.avgf  # Segment length
        self.original_dim = self.input.shape[-1] #Expected 504
        self.target_dim = 240

        if self.input.shape[1] % self.avgf != 0 or int(self.seg) == 0:
            raise ValueError(f"ERROR 3 - TTM: self.seg = {self.seg} != {self.input.shape[1]}/{self.avgf}")

        # **Fix: Ensure Correct Embedding Size**
        self.M_size1 = self.input.shape[-1]  # Corrected from `self.input.shape[2] * self.input.shape[3]`
        self.tK = num_blocks
        self.hA = num_heads
        self.Dh = int(self.M_size1 / self.hA)

        if self.M_size1 % self.hA != 0 or self.Dh == 0:
            raise ValueError(f"ERROR 4 - TTM: self.Dh = {self.Dh} != {self.M_size1}/{self.hA}. Try different num_heads.")

        # **Fix: Correct Weight Initialization**
        self.weight = nn.Parameter(torch.randn(self.input.shape[0], self.M_size1, self.M_size1, dtype=self.dtype))
    ...
    def forward(self, x):
        # Transform Input to [D, S, C]
        input = x.transpose(1, 3)  

        # Initialize inputc
        inputc = torch.zeros(input.shape[0], self.avgf, input.shape[2], input.shape[3], dtype=self.dtype).to(device)

        # **Segment-wise aggregation**
        for b in range(input.shape[0]):
            for i in range(self.avgf):
                for j in range(int(i * self.seg), int((i + 1) * self.seg)):
                    inputc[b, i, :, :] += input[b, j, :, :]
                inputc[b, i, :, :] /= self.seg

        # **Ensure altx has the correct shape**
        altx = inputc.reshape(input.shape[0], self.avgf, self.M_size1).to(device)  # [batch, M, S*C]

        # **Fix: Ensure `altx` matches `self.weight`**
        expected_m = self.weight.shape[-1]  # Get expected size (should be 504)
        current_m = altx.shape[-1]

        if current_m != expected_m:
            print(f"⚠ WARNING: Mismatch in m dimension! Adjusting from {current_m} to {expected_m}")
            if current_m < expected_m:
                # Zero-pad if altx is smaller than expected
                altx = torch.nn.functional.pad(altx, (0, expected_m - current_m))
            else:
                # Trim if altx is larger than expected
                altx = altx[:, :, :expected_m]

        # **Fix: Perform einsum with correctly sized matrices**
        savespace = torch.einsum('blm,bim -> bil', self.weight, altx)
        ...
        # **Fix: Ensure correct reshaping at the end**
        expected_shape = (input.shape[0], self.avgf + 1, input.shape[2], savespace.shape[-1])
        print(f"Attempting reshape with shape: {expected_shape}")
        final_output = savespace.view(expected_shape)

        print(f'Fixed final output shape: {final_output.shape}')
        return final_output
...
class CNNdecoder(nn.Module):  # EEGformer decoder
    def __init__(self, input, num_cls, CF_second, dtype):  
        ...
        self.cvd2 = nn.Conv1d(in_channels=1, out_channels=self.n, kernel_size=1, dtype=self.dtype)  # Input: [B, S, M]

        # Third convolution: Handles the `M` dimension
        self.cvd3 = nn.Conv1d(in_channels=self.m, out_channels=60, kernel_size=1, dtype=self.dtype)  # Use 60 instead of `int(self.m / 2)`

        # Fully connected layer
        self.fc = nn.Linear(60 * self.n, num_cls, dtype=self.dtype)
    ...
    def forward(self, x):  # x -> [B, M, S, C]
        ...
        print(f'Fixed CNN Decoder Input Shape: {x.shape}')
...
class EEGformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.lstm(x)
        print(f"Output shape from LSTM: {x.shape} (expected: [batch_size, reduced_timesteps, embedding_dim])")

        x = self.ttm(x)
        print(f"Output shape from TTM: {x.shape} (expected: [batch_size, submatrices, S, embedding_dim])")
