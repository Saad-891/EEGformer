...
class ODCM(nn.Module):
    ...
    def forward(self, x):
        #print("Input ODCM: ", x.shape)
        x = self.cvf1(x)
        #print("Post cvf1: ", x.shape)
        x = self.relu(x)
        x = self.cvf2(x)
        #print("Post cvf2: ", x.shape)
        x = self.relu(x)
        x = self.cvf3(x)
        #print("Post cvf3: ", x.shape)
        x = self.relu(x)

        B, Cncf, T = x.shape
        expected_channels = self.ncf * self.inpch

        if Cncf != expected_channels:
            raise ValueError(f"[ODCM] Expected {expected_channels} channels (ncf × input_channels), got {Cncf}.")

        x = x.view(B, self.inpch, self.ncf, T)  # [B, C, ncf, T]
...
class RTM(nn.Module):  # Regional transformer module
    def __init__(self, input, num_blocks, num_heads, dtype):  # input -> [B, C, D, S]
        super(RTM, self).__init__()
        if input.dim() != 4:
            raise ValueError(f"[RTM] Expected 4D input, got shape {input.shape}")

        # Save shape without unnecessary transposes
        self.inputshape = input.shape  # [B, C, D, S]
        self.M_size1 = self.inputshape[3]  # S (Time steps)
        self.dtype = dtype
        ...
        if self.M_size1 % self.hA != 0 or self.Dh == 0:
            print(f"❌ ERROR 1 - RTM: self.Dh = {self.Dh} != {self.M_size1}/{self.hA} \n⚠️ Try with different num_heads")

        print(f"[RTM Init] Input shape: {self.inputshape} | Time Steps (S): {self.M_size1} | num_heads: {self.hA} | Dh: {self.Dh}")

        # weight: [B, D, S]
        self.weight = nn.Parameter(torch.randn(self.inputshape[0], self.inputshape[1], self.inputshape[3], dtype=self.dtype))
        # bias: [B, S, C+1, D]
        self.bias = nn.Parameter(torch.zeros(self.inputshape[0], self.inputshape[3], 2, self.inputshape[2], dtype=self.dtype))
        self.cls = nn.Parameter(torch.zeros(self.inputshape[0], self.inputshape[3], 1, self.inputshape[2], dtype=self.dtype))  # [B, S, 1, D]

        trunc_normal(self.bias, std=.02)
        trunc_normal(self.cls, std=.02)

        self.tfb = nn.ModuleList([GenericTFB(self.inputshape[1], self.hA, self.dtype) for _ in range(self.tK)])

    def forward(self, x):  # x: [B, C, D, S]
        print(f"\n[RTM] Input to forward(): {x.shape}")

        # Permute: [B, C, D, S] → [B, D, S, C]
        x = x.permute(0, 2, 3, 1)
        print(f"[RTM] After permute (B, D, S, C): {x.shape}")
        print(f"[RTM] self.weight: {self.weight.shape}")

        # Apply einsum: [B, D, S], [B, D, S, C] → [B, S, C, D]
        savespace = torch.einsum('bds,bdsc->bscd', self.weight, x)
        print(f"[RTM] After einsum (B, S, C, D): {savespace.shape}")

        # Add CLS token: [B, S, 1, D]
        print(f"[RTM] CLS shape: {self.cls.shape}")
        # Add CLS token: [B, S, 1, D]
        savespace = torch.cat([self.cls, savespace], dim=2)  # [B, S, C+1, D]
        print(f"[RTM] After CLS concat (B, S, C+1, D): {savespace.shape}")

        # Add bias
        print(f"[RTM] Bias shape: {self.bias.shape}")
        savespace = savespace + self.bias
        print(f"[RTM] After adding bias: {savespace.shape}")

        # Apply transformer blocks
        for i, tfb in enumerate(self.tfb):
            print(f"[RTM] Passing through Transformer Block {i+1}")
            savespace = tfb(x, savespace)

        print(f"[RTM] Output shape: {savespace.shape}")
        return savespace  # [B, S, C+1, D]
...
class EEGformer(nn.Module):
    def __init__(self, input, num_cls, input_channels, kernel_size, num_blocks, num_heads_RTM, num_heads_STM, num_heads_TTM, num_submatrices, CF_second, dtype=torch.float32):
        ...
        B = input.shape[0]
        T = input.shape[-1]
        self.outshape1 = torch.zeros(B, self.input_channels, self.ncf, T - 3 * (self.kernel_size - 1)).to(device)
